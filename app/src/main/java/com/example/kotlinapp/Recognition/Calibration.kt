...
class Calibration(internal var mainActivity: MainActivity) {
    ...
    fun getColor(image: Mat): String? {//Scalar? {
        this.image = image
        when (colorNumber) {
            ...
            0 -> return "Tap on the red color on the cube"
            1 -> return "Tap on the green color on the cube"
            2 -> return "Tap on the orange color on the cube"
            3 -> return "Tap on the yellow color on the cube"
            4 -> return "Tap on the white color on the cube"
            5 -> return "Tap on the blue color on the cube"
            else -> return "Calibration finished! Tap to continue."
        }
        //return null
        return ""
    }
    ...
    fun getColorByCoordinates(tileCenter: Point) {
        ...
        val color =
            android.graphics.Color.rgb(
                measuredColorArray[0].toFloat(),
                measuredColorArray[1].toFloat(),
                measuredColorArray[2].toFloat()
            )
        when (colorNumber) {
            0 -> {
                Color.RED.hsvCalibrateValue =
                    Scalar(
                        measuredColorArray[0],
                        measuredColorArray[1],
                        measuredColorArray[2],
                        measuredColorArray[3]
                    )
            ...
            5 -> {
                ...
                mainActivity.updateCalibrationText(null)
            }
        }
    }
}